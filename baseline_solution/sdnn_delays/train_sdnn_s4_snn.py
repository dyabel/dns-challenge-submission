...
import importlib
from s4.s4_model_snn import S4Model
...
class Network(torch.nn.Module):
    def __init__(self, threshold=0.1, tau_grad=0.1, scale_grad=0.8, max_delay=64, out_delay=0, args=None):
        ...
        #                 prenorm=False,
        #                 args=args) # Build Neural Network
        self.decoder = nn.Linear(256, 257)
        self.time_window = 5
...
if __name__ == '__main__':
    ...
    trained_folder = 'checkpoints/Trained' + identifier
    logs_folder = 'logs/Logs' + identifier
    ...
    for epoch in range(args.epoch):
        ...
        for i, (noisy, clean, noise) in enumerate(validation_loader):
            ...
            with torch.no_grad():
                ...
                clean_rec = stft_mixer(denoised_abs, noisy_arg, args.n_fft)
                # if i % 100 == 0:
                #     pesq = np.mean([pysepm.pesq(clean1, clean_rec1, fs)[1] for clean1, clean_rec1 in zip(clean.detach().cpu().numpy(), clean_rec.detach().cpu().numpy())])
                #     fwsnrseg = np.mean([pysepm.fwSNRseg(clean1, clean_rec1, fs) for clean1, clean_rec1 in zip(clean.detach().cpu().numpy(), clean_rec.detach().cpu().numpy())])
                #     stoi = np.mean([pysepm.stoi(clean1, clean_rec1, fs) for clean1, clean_rec1 in zip(clean.detach().cpu().numpy(), clean_rec.detach().cpu().numpy())])
