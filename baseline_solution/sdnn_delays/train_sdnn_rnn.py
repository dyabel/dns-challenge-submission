...
class Network(torch.nn.Module):
    def __init__(self, threshold=0.1, tau_grad=0.1, scale_grad=0.8, max_delay=64, out_delay=0):
        ...
        self.proj = Project()
        self.proj.additem('input_size', 257)
        self.proj.additem('num_classes', 257)
        self.proj.select_device()  # Select CPU/GPU
        self.proj.build_structure()
        ...
        # ])
        # self.net = importlib.import_module('deltarnn.networks.models.cla-deltarnn').Model(self.proj)
        self.net = self.proj.build_model()  # Build Neural Network

        self.train_func = importlib.import_module('deltarnn.modules.gscdv2.train_func')
    ...
    def forward(self, noisy):
        ...
        x = x.transpose(1, 2)
        x, reg = forward_propagation(self.net, x)
