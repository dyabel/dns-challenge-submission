...
class FRCRN(nn.Module):
    ...
    def __init__(self,
        ...
        self.s4_real1 = S4Model(d_input=321,
                        d_output=321,
                        d_model=256,
                        n_layers=4,
                        dropout=0.2,
        ...
        self.s4_real2 = S4Model(d_input=321,
                        d_output=321,
                        d_model=256,
                        n_layers=4,
                        dropout=0.2,
        ...
        self.s4_imag1 = S4Model(d_input=321,
                        d_output=321,
                        d_model=256,
                        n_layers=4,
                        dropout=0.2,
        ...
        self.s4_imag2 = S4Model(d_input=321,
                        d_output=321,
                        d_model=256,
                        n_layers=4,
                        dropout=0.2,
    ...
    def forward(self, inputs):
        ...
        # [B, 2, T, D]
        cmp_spec = torch.transpose(cmp_spec, 2, 3)
        s4_out1 = torch.cat([self.s4_real1(cmp_spec[:,0,:,:]).unsqueeze(1), self.s4_imag1(cmp_spec[:,1,:,:]).unsqueeze(1)], dim=1)
        cmp_mask1 = torch.tanh(s4_out1).unsqueeze(-1).transpose(1,4).transpose(2,3)
        s4_out2 = torch.cat([self.s4_real2(s4_out1[:,0,:,:]).unsqueeze(1), self.s4_imag2(s4_out1[:,1,:,:]).unsqueeze(1)], dim=1)
        cmp_mask2 = torch.tanh(s4_out2).unsqueeze(-1).transpose(1,4).transpose(2,3)
        cmp_spec = cmp_spec.unsqueeze(-1).transpose(1,4).transpose(2,3)
