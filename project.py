...
from deltarnn.utils import pandaslogger, util
...
class Project:
    ...
    def load_config(self):
        config_path = os.path.join('./deltarnn/config', self.dataset, self.cfg_feat + '.json')
    ...
    def load_modules(self):
        ...
        #     raise RuntimeError('Please select a supported dataset.')
        try:
            self.log = importlib.import_module('deltarnn.modules.' + self.dataset + '.log')
        except:
            raise RuntimeError('Please select a supported dataset.')
        # try:
        ...
        #     raise RuntimeError('Please select a supported dataset.')
        try:
            self.net_pretrain = importlib.import_module('deltarnn.networks.models.' + self.model_pretrain)
        except:
            raise RuntimeError('Please select a supported dataset.')
        try:
            self.net_retrain = importlib.import_module('deltarnn.networks.models.' + self.model_retrain)
        except:
            raise RuntimeError('Please select a supported dataset.')
    ...
    def build_model(self):
        from deltarnn.utils.util import count_net_params
