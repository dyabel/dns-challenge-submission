...
class GatedMultiScaleRetentionLayer(nn.Module):
    def __init__(
        ...
        self.gamma = nn.Parameter(1 - torch.exp(
            (-5 - torch.arange(0, num_heads)) * torch.log(torch.tensor(2))), requires_grad=False
        )  # shape: [num_heads]
    ...
    def forward(self, x, past_kv=None, paradigm="parallel", chunk_index=0):
        ...
        if paradigm == "parallel":
            ...
            decay_mask = self.get_masks(seq_len=seq_len)
            inner_decay = self.gamma.unsqueeze(1) ** (torch.arange(seq_len) + 1).to(self.gamma.device)
            chunk_decay = self.gamma.view(-1, 1, 1) ** (seq_len)
            inner_decay = inner_decay.to(q.device)
            chunk_decay = chunk_decay.to(q.device)
    ...
    def get_masks(self, seq_len):
        row_indices = torch.arange(seq_len).view(-1, 1).to(self.gamma.device)
        column_indices = torch.arange(seq_len).view(1, -1).to(self.gamma.device)

        power_matrix = row_indices - column_indices
        gamma_expanded = self.gamma.view(-1, 1, 1)
        masks = torch.where(
            power_matrix >= 0, gamma_expanded**power_matrix, torch.zeros(1, 1).to(self.gamma.device)
        )
        return masks
        ## Normalize Masks
